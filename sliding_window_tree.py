        ...
        if rank == MASTER_RANK:
            ...
            while start_window_nucpos <= last_window_start_nucpos or busy_slave_2_request:
                ...
                while start_window_nucpos <= last_window_start_nucpos and available_slaves:
                    ...
                    str_window_args = ''.join(window_args_arr)
                    LOGGER.debug("Sending window_args to slave=" + str(slave_rank) + " " + str_window_args)

                    comm.Isend(buf=[window_args_arr, MPI.CHAR], dest=slave_rank, tag=TAG_WORK)  # non-blocking
                    empty_buffer = array.array('c', '\0'*2046)
                    request = comm.Irecv(buf=[empty_buffer, MPI.CHAR], source=slave_rank, tag=MPI.ANY_TAG)  # non-blocking

                    # The memory containing the window arguments must be kept intact until the slave says they're done
                    # otherwise race condition can occur when memory is overwritten and slave no longer has access to args.
                    # However, mpi4py will auto-allocate memory to contain the slave response message.
                    busy_slave_2_request[slave_rank] = WindowSlaveInfo(slave_rank=slave_rank, work_arguments=window_args_arr,
                                                                       mpi_request=request)
                ...
                if busy_slave_2_request:
                    requests = [window_slave_info.mpi_request for window_slave_info in busy_slave_2_request.values()]
                    mpi_status = MPI.Status()
                    idx, err_msg = MPI.Request.waitany(requests=requests, status=mpi_status)  # Block till any slave returns

                    slave_rank = mpi_status.Get_source()
                    if err_msg:
                        LOGGER.error("Received error from slave=" + str(slave_rank) + " err_msg=" + str(err_msg))
                    else:
                        LOGGER.debug("Received success from slave=" + str(slave_rank))
            ...
            while not is_die:
                try:
                    ...
                    rcv_buffer = array.array('c', '\0'*4096)
                    ...
                    if mpi_status.Get_tag() == TAG_DIE:  # master wants me to die
                        ...
                        process_args["window_depth_cutoff"] = process_args_arr[1]
                        process_args["window_breadth_cutoff"] = process_args_arr[2]
                        process_args["start_window_nucpos"] = int(process_args_arr[3])
                        process_args["end_window_nucpos"] = int(process_args_arr[4])
                        process_args["pvalue"] = float(process_args_arr[5])
                        process_args["threads_per_window"] = int(process_args_arr[6])
