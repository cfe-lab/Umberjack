...
def calc_full_popn_section_dnds(section_range, outdir, filename_prefix):
    """
    For a recombinant section of the full population, calculate the dnds
    :param tuple section_range:  (start,  end) 1based nuc position of recombinant section
    :param str outdir:  output directory
    :param str filename_prefix:  prefix of basename of file (no file suffix or directory path)
    """
    start, end = section_range
    full_popn_section_fasta = outdir + os.sep + "fullpopn" + os.sep + filename_prefix + ".break.{}_{}.fasta".format(start, end)
    full_popn_section_treefile = outdir + os.sep + "topology" + os.sep + filename_prefix + ".break.{}_{}.rename.nwk".format(start, end)

    out_inferred_nwk = outdir + os.sep + "subs" + os.sep + filename_prefix + ".break.{}_{}.fasttree.nwk".format(start, end)
    out_hyphy_prefix = outdir + os.sep + "subs" + os.sep + filename_prefix + ".break.{}_{}".format(start, end)

    # Use FastTree to recalculate the branch lengths constrained by the same topology as the asg_driver.py topologies
    fasttree_handler.make_tree_repro(fasta_fname=full_popn_section_fasta,
                                     intree_fname=full_popn_section_treefile,
                                     outtree_fname=out_inferred_nwk,
...
if __name__ == "__main__":
    ...
    # For each recombinant section of the full population genome,
    # Infer the tree branch lengths from the full population sequences, constraining the topology by that generated by the asg_driver.py.
    # Calculate site dnds using the recombinant section sequences and inferred tree
    ...
    # Hyphy doesn't use up more than 4 threads.
    # Speed this up by running in parallel as much as possible.
    # Reproducible FastTree must be single threaded
    # HyPhy can only use up to 4 threads at once.
    part_calc_full_popn_section_dnds = functools.partial(calc_full_popn_section_dnds, outdir=OUTDIR, filename_prefix=FILENAME_PREFIX)
    total_concurrent_hyphy = max(PROCS/4, PROCS)
    thepool = pool.pool_traceback.LoggingPool(total_concurrent_hyphy)
    for result in thepool.imap_unordered(part_calc_full_popn_section_dnds, recombo_sections):
        pass
    thepool.close()
