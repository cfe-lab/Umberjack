...
#
#
# def eval_windows_mpi(out_dir, map_qual_cutoff, read_qual_cutoff, max_prop_n, start_nucpos, end_nucpos, window_size,
#                      window_depth_cutoff, window_breadth_cutoff, window_slide, insert, mask_stop_codon,
#                      remove_duplicates, output_csv_filename, mode, threads_per_window, hyphy_exe, hyphy_basedir,
#                      hyphy_libdir, fastree_exe, debug, ref="", sam_filename="", sam_filename_list=""):
#     """
#     Launch a separate process to analyze each window via MPI.  Similar to eval_windows_async, but uses MPI.
#
#     :param sam_filename_list:
#     :param str ref:  reference name
#     :param str sam_filename:  full filepath to sam file of read alignments against the reference
#     :param str out_dir:  output directory
#     :param int map_qual_cutoff:  mapping quality threshold below which alignments are thrown out
...
#
#
#     check_sams(sam_filename=sam_filename, sam_filename_list=sam_filename_list, ref=ref)
#
#
...
#                 os.makedirs(out_dir)
#
#             if not sam_filename_list:
#                 queue_samfiles = [sam_filename]
#             else:
#                 queue_samfiles = sam_filename_list
#
...
#                     LOGGER.debug("Queuing sam " + queue_next_samfile + " ref " + queue_next_ref)
#
#                     if not start_nucpos:
#                         start_nucpos = 1
#                     if not end_nucpos:
#                         end_nucpos = sam_handler.get_reflen(queue_next_samfile, queue_next_ref)
#
#                     if debug:
#                         create_full_msa_fasta(sam_filename=queue_next_samfile, out_dir=out_dir, ref=queue_next_ref,
#                                               mapping_cutoff=map_qual_cutoff, read_qual_cutoff=read_qual_cutoff,
#                                               is_insert=insert, is_mask_stop_codon=mask_stop_codon)
#
#                     if remove_duplicates:
#                         create_dup_tsv(sam_filename=queue_next_samfile, out_dir=out_dir, ref=queue_next_ref,
#                                        mapping_cutoff=map_qual_cutoff, read_qual_cutoff=read_qual_cutoff,
#                                        is_insert=insert)
#
#                     # All nucleotide positions are 1-based
#                     total_windows = int(math.ceil(((end_nucpos-window_size+1) - start_nucpos + 1)/window_slide))
#                     LOGGER.debug("Launching " + str(total_windows) + " total windows for sam=" + queue_next_samfile + " ref=" + queue_next_ref)
#
#                     # Assign work to replicas
#                     for start_window_nucpos in range(start_nucpos, end_nucpos-window_size-1, window_slide):
...
#
#
#                     LOGGER.debug("Done Launching " + str(total_windows) + " total windows for sam=" + queue_next_samfile + " ref=" + queue_next_ref)
#
...
def main():
    ...
    parser.add_argument("--mode", default=settings.DEFAULT_MODE, choices=[UmberjackPool.MODE_DNDS, UmberjackPool.MODE_GTR_RATE, UmberjackPool.MODE_COUNT_SUBS],
    ...
    if args.mpi:
        ...
        umberjackworker = OneNodeUmberjackPool(**eval_windows_args)
